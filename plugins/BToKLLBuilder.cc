...
void BToKLLBuilder::produce(edm::StreamID, edm::Event &evt, edm::EventSetup const &) const {
  ...
  for(size_t k_idx = 0; k_idx < kaons->size(); ++k_idx) {
    ...
    for(size_t ll_idx = 0; ll_idx < dileptons->size(); ++ll_idx) {
      ...
      // Aqui creemos el boost al CM del dilepton post fitting
      // math::XYZTLorentzVector dilep_post = fitter.daughter_p4(0).globalMomentum()+fitter.daughter_p4(1).globalMomentum();
      // std::cout << "\nLeptones extraidos\n";
      // ROOT::Math::Boost cmboost(dimuon_post.BoostToCM());
      // math::XYZTLorentzVector kaonCM(  cmboost( k_ptr->p4() )  );
      // math::XYZTLorentzVector muonCM1, muonCM2;
      // if (l1_ptr->charge()==-1){
      //   muonCM1 = cmboost(l1_ptr->p4()) ;
      ...
      // cand.addUserFloat("post_cosTheta_mm", costhetaL);
      // cand.addUserFloat("post_cosTheta_km", costhetaKL);
